...
reset:
  ...
  lda #0
  sta counter_one
  sta counter_one + 1	; Sett begge counter bytes til 0
...
loop:
  ...
  lda counter_one		; hent tall
  sta value			; lagre i RAM
  lda counter_one + 1	; hent neste byte
...
irq:
  ...
  lda PORTA			; Les PORTA, behandle data, fjerner interrupt signalet
  cmp #%00000010	; sjekk om bit 7 (venstre) er HØY
  beq inc_counter	; legg til 1 i counter
  cmp #%00000100	; sjekk om bit 6 (top) er HØY
  beq inc_counter	; legg til 1 i counter
...
exit_irq:
  ...
  ldy #$2f
  ldx #$ff			; last inn stor verdi i x registerets
delay:
...
inc_counter:
  inc counter_one		; legg til 1 i counter
  bne exit_irq		; hvis den ikke ruller over
  inc counter_one + 1	; legg til 1 i bit-en etter counter
